...
/* non-static: tr_gx2_backend.c generates the GX2 mip chain with it */
...
*/
#if defined(WIIU_GX2BE)
/* Set by R_CreateImage before calling Upload32 so the GX2 upload hooks
 * inside Upload32 know the destination slot and wrap mode without
 * changing Upload32's upstream signature. */
static int s_gx2UploadTexnum = -1;
static int s_gx2UploadWrap   = GL_REPEAT;
#endif
						  ...
						  int *pUploadWidth, int *pUploadHeight )
{
	...
	if ( ( scaled_width == width ) && 
		...
		{
#if defined(WIIU_GX2BE)
			GX2BE_Upload32( data, scaled_width, scaled_height,
			               (int)internalFormat, s_gx2UploadTexnum, s_gx2UploadWrap,
			               qfalse, qtrue );
#else
	...
	*format = internalFormat;
	 ...
	 * When mipmap is set, GX2BE_Upload32 generates the full chain itself
	 * (R_MipMap on its own staging copy — scaledBuffer is not mutated). */
	GX2BE_Upload32( scaledBuffer, scaled_width, scaled_height,
	               (int)internalFormat, s_gx2UploadTexnum, s_gx2UploadWrap,
	               mipmap, qtrue );
#else
...
image_t *R_CreateImage( const char *name, byte *pic, int width, int height,
...
#if defined(WIIU_GX2BE)
	GX2BE_CreateTexnum( &image->texnum );
#else
...
#if defined(WIIU_GX2BE)
	/* The GX2 upload happens INSIDE Upload32 (at the level-0 upload points),
	...
	s_gx2UploadTexnum = (int)image->texnum;
	s_gx2UploadWrap   = glWrapClampMode;
#endif
...
void R_DeleteTextures( void ) {
	...
	for ( i=0; i<tr.numImages ; i++ ) {
#if defined(WIIU_GX2BE)
		GX2BE_DeleteTexnum( (int)tr.images[i]->texnum );
#else

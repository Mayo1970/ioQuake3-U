...
static void DrawSkySide( struct image_s *image, const int mins[2], const int maxs[2] )
{
	...
	{
#if defined(WIIU_GX2BE)
		/* count must exactly match the emitted vertices */
		GX2BE_ImmediateBegin( GL_TRIANGLE_STRIP, ( maxs[0] - mins[0] + 1 ) * 2 );

		for ( s = mins[0]+HALF_SKY_SUBDIVISIONS; s <= maxs[0]+HALF_SKY_SUBDIVISIONS; s++ )
		{
			GX2BE_ImmediateTexVertex( s_skyTexCoords[t][s], s_skyPoints[t][s], c, c, c, 255 );
			GX2BE_ImmediateTexVertex( s_skyTexCoords[t+1][s], s_skyPoints[t+1][s], c, c, c, 255 );
		}
...
#if defined(WIIU_GX2BE)
	GX2BE_LoadModelviewTranslatedGL( backEnd.viewParms.world.modelMatrix,
	                                backEnd.viewParms.or.origin );
#else

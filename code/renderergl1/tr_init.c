...
static void InitOpenGL( void )
{
	...
	if ( glConfig.vidWidth == 0 )
	{
		...
		/* No GL to query. 2048 keeps worst-case RGBA8 mip chains ~21.8 MB;
		 * Q3 assets are <=512 anyway. */
		(void)temp;
...
#if defined(WIIU_GX2BE)
	GX2BE_ReadPixelsRGB(x, y, width, height, padwidth - linelen, bufstart);
#else
...
#if defined(WIIU_GX2BE)
	GX2BE_ReadPixelsRGB(0, 0, cmd->width, cmd->height, padlen, cBuf);
#else
...
{
#if defined(WIIU_GX2BE)
	GX2BE_SetDefaultState();

	/* keep the frontend's shadow state in sync (same as the GL arm below) */

...
int Export_BotLibLoadMap(const char *mapname)
{
	...
	//
#ifdef __WIIU__
	/* Not declared in be_aas_main.h -- only ever called internally from
	 * that file's own AAS_StartFrame before now. */
	extern void AAS_ContinueInit(float time);

	/* Force the reachability+routing build to finish synchronously here
	 * instead of trickling across server frames via AAS_StartFrame -- see
	 * wiiu-bot-autospawn-shelved memory. This trades a load-time stall
	 * (observed up to ~30s on real hardware for mid-sized maps) for
	 * removing the AAS-not-ready race against addbot entirely. The `time`
	 * arg is unused by AAS_ContinueInit/AAS_ContinueInitReachability on
	 * this path (they track wall time via Sys_MilliSeconds internally),
	 * so 0 is fine. PS3/vWii/stock keep the normal per-frame path. */
	while (aasworld.loaded && !aasworld.initialized) {
		AAS_ContinueInit(0);
	}
#endif

...
#ifdef __WIIU__
    /* This read-ahead buffer is private scratch, not shared game state --
     * keep it off the fixed-size mainzone (com_zoneMegs, 24 MB, hard-crashes
     * via Com_Error when exhausted) and pull from the general ~800+ MB heap
     * headroom instead, so UNZ_BUFSIZE can be sized generously without
     * risking zone exhaustion. */
...
#ifdef __WIIU__
    if (pfile_in_zip_read_info->read_buffer) free(pfile_in_zip_read_info->read_buffer);
#else

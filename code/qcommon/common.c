...
#ifndef DEDICATED
#ifdef __WIIU__
char	cl_cdkey[34] = "wj7cplhs2gp3ac3a                "; // __WIIU__: seed valid key, skip CD-Key entry menu (no keyboard at that point)
#else
char	cl_cdkey[34] = "                                ";
#endif
#else
...
void Com_ReadCDKey( const char *filename ) {
	...
	if ( !f ) {
#ifdef __WIIU__
		// no q3key on disk yet (first boot) -- seed a known-valid key so
		// CL_CDKeyValidate passes and the CD-Key entry menu never shows
...
static void Com_WriteCDKey( const char *filename, const char *ikey ) {
	...
	char			fbuffer[MAX_OSPATH];
	char			key[17];
#if !defined(_WIN32) && !defined(__WIIU__) // wut newlib has no umask()
	mode_t			savedumask;
#endif

...
/* CLASSIC (Dreamcast Q3 1.13n crossplay): primary protocol IS 43, legacy
 * version matches so clc.compat fires automatically -- same PRIMARY==LEGACY
 * trick OA uses below. */
#if defined(CLASSIC)
...
#endif
/* OA's actual wire protocol is the old idQuake3 "legacy" framing at version
 * 71 (not ioquake3's enhanced format) -- matching PRIMARY==LEGACY makes
 * cl_main.c's CL_CheckForResend force clc.compat=qtrue unconditionally
 * (com_legacyprotocol->integer == com_protocol->integer), which is the only
 * way the client ever sends the legacy wire format at all. Without this,
 * OA's server rejects every connect with "yours is 68" since the client

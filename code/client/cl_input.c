...
#if defined(__WIIU__)
	/* Let the normal Controls-menu sensitivity slider (cl_sensitivity) scale
	 * stick yaw/pitch too, not just mouse -- matches the vWii sibling port's
	 * hardware-validated GEKKO block (code/client/cl_input.c in
	 * ioquake3-wii), which fixed the same "menu slider does nothing for
	 * stick players" gap. */
	{
...
#ifdef __WIIU__
/* IR fine-aim offset (set by wiiu_input.c) applied on top of cl.viewangles here,
 * separate from the continuous turn-rate that rotates cl.viewangles itself. */
extern float wiiu_ir_aim_x;
extern float wiiu_ir_aim_y;
extern cvar_t *pointer_yawrange;
extern cvar_t *pointer_pitchrange;
#endif
...
#ifdef __WIIU__
	{
		float yawR   = pointer_yawrange   ? pointer_yawrange->value   : 50.0f;
		float pitchR = pointer_pitchrange ? pointer_pitchrange->value : 30.0f;
		float inv    = (m_pitch->value >= 0.0f) ? 1.0f : -1.0f;
		float yaw    = cl.viewangles[YAW]   - wiiu_ir_aim_x * yawR;
		float pitch  = cl.viewangles[PITCH] + wiiu_ir_aim_y * pitchR * inv;

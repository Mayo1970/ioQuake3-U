...
#ifdef __WIIU__
extern void WiiU_SetRumble(int durationMs);
#endif
 ...
 * The Wii U GamePad motor is on/off only (no dual-motor amplitude like
 * DualShock4), so each category maps to a pulse duration instead of a
 * large/small motor pair. Durations are a first cut, ported from the PS4
 * port's sustain times, and are tuned by feel on real hardware. */
static void WiiU_RumbleForSfx(const char *name, int entityNum, qboolean localSound)
{
	...
	if (!Q_stricmpn(name, "sound/weapons/", 14)) {
		if (strstr(name, "impact") || strstr(name, "explod"))
			return;
		if (strstr(name, "/rocket/"))     { WiiU_SetRumble(250); return; }
		if (strstr(name, "/shotgun/"))    { WiiU_SetRumble(150); return; }
		if (strstr(name, "/machinegun/")) { WiiU_SetRumble(100); return; }
		if (strstr(name, "/vulcan/"))     { WiiU_SetRumble(100); return; }
		if (strstr(name, "/railgun/"))    { WiiU_SetRumble(200); return; }
		if (strstr(name, "/nailgun/"))    { WiiU_SetRumble(150); return; }
		if (strstr(name, "/plasma/"))     { WiiU_SetRumble(100); return; }
		if (strstr(name, "/proxmine/"))   { WiiU_SetRumble(200); return; }
		if (strstr(name, "/grenade/"))    { WiiU_SetRumble(150); return; }
		/* Melee doesn't work with rumble yet — no sound file is registered
		 * when the gauntlet fires, so this branch is never reached. */
		if (strstr(name, "/melee/"))      { WiiU_SetRumble(120); return; }
		if (strstr(name, "/lightning/"))  { WiiU_SetRumble(150); return; }
		if (strstr(name, "/bfg/"))        { WiiU_SetRumble(220); return; }
		WiiU_SetRumble(90);
		return;
	}
}
...
#ifdef __WIIU__
	WiiU_RumbleForSfx(sfx->soundName, entityNum, localSound);
#endif

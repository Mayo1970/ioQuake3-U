...
#elif defined(ELITEFORCE)
		// EF servers parse the connect packet as a plain OOB string, not
		// ioquake3's huffman-compressed OOB data block.
		NET_OutOfBandPrint( NS_CLIENT, clc.serverAddress, "%s", data );
#else
		NET_OutOfBandData( NS_CLIENT, clc.serverAddress, (byte *) data, strlen ( data ) );
#endif
...
void CL_Init( void ) {
...
#if defined(__WIIU__)
	/* Lower out-of-box default for stick-only play (analog sticks read as
	 * full-range joystick axes, unlike a mouse) -- only affects a fresh
	 * config; Cvar_Get preserves an existing archived value, so changing
	 * this in the Controls-menu slider still persists across boots. */
	...
	// userinfo
#ifdef __WIIU__
	// Mii nickname is the platform's notion of a player name -- make it the actual
	// default so "Defaults" in the setup menu (cvar_restart) restores it too.
	Cvar_Get ("name", WiiU_DefaultPlayerName(), CVAR_USERINFO | CVAR_ARCHIVE );
#else

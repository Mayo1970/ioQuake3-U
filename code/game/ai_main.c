...
// BotAIStartFrame's think-scheduling epoch: this port compiles qagame
// natively, so these survive across G_InitGame calls as ordinary BSS
// instead of getting a fresh zeroed data segment like stock ioquake3's
// QVM/DLL game module; must be reset explicitly in BotAISetup or a stale
// large value from the previous map's level.time epoch makes elapsed_time
// go deeply negative on the first frame of every subsequent map, and bots
// never think again
...
int BotAISetup( int restart ) {
	...
	// level.time resets unconditionally in G_InitGame on both a real map
	// change and a map_restart, so BotAIStartFrame's think-scheduling epoch
	// must reset unconditionally too, unlike botstates[] below which is
	// intentionally preserved across a restart
	local_time = 0;
	botlib_residual = 0;
	lastbotthink_time = 0;
...
int BotAIShutdown( int restart ) {
	...
	if ( restart ) {
		...
		//shut down every bot's client-side state (botstates[]) while the
		//botlib is still alive, or it survives into the next game: the
		//botlib's own handle tables get wiped by trap_BotLibShutdown
		//below, so a leftover bot_state_t with inuse still true makes
		//BotAISetupClient reject the next game's setup as "already
		//setup" and any handles left inside it are already stale.
		for (i = 0; i < MAX_CLIENTS; i++) {
			if (botstates[i] && botstates[i]->inuse) {
				BotAIShutdownClient(botstates[i]->client, restart);
			}

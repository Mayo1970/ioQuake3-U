...
#ifndef __WIIU__
/* WIIU: qagame is statically linked into the same binary as the engine
 * (Makefile.client GAME_SRCS / VMI_NATIVE, see code/wiiu/sys_wiiu.c) — the
 * real Com_Error/Com_Printf (code/qcommon/common.c) are already in this
 * link, same signatures, so any game code calling them lands directly on
 * the engine's implementation instead of needing this trap-bounce shim. */

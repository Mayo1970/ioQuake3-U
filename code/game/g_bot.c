...
/* Wii U-only: qagame is statically linked (never reloaded between maps,
 * see code/wiiu/sys_wiiu.c's Sys_LoadGameDll), so unlike stock's fresh
 * QVM/DLL per map, botlib's AAS_ContinueInit route-cache build is genuinely
 * slower wall-clock here (weaker CPU) and can still be running when an
 * addbot request comes in at map-load time. Stock has no retry for this —
 * BotAISetupClient just fails and the request is lost. Queue it instead. */
#define PENDING_BOT_ADD_DEPTH		8
#define PENDING_BOT_ADD_TIMEOUT		30000
...
static pendingBotAdd_t	pendingBotAdds[PENDING_BOT_ADD_DEPTH];

// G_CheckMinimumPlayers's throttle: this port compiles qagame natively, so
// this survives across G_InitGame calls as ordinary BSS instead of getting
// a fresh zeroed data segment like stock ioquake3; reset in G_InitBots or a
// stale large value from the previous map's level.time epoch suppresses the
// check for up to ~10s of accumulated playtime after every map change
...
/*
===============
G_QueuePendingBotAdd
===============
*/
static void G_QueuePendingBotAdd( const char *name, float skill, const char *team, int delay, char *altname ) {
	int		n;

	for( n = 0; n < PENDING_BOT_ADD_DEPTH; n++ ) {
		if( !pendingBotAdds[n].active ) {
			pendingBotAdds[n].active = qtrue;
			Q_strncpyz( pendingBotAdds[n].name, name, sizeof(pendingBotAdds[n].name) );
			pendingBotAdds[n].skill = skill;
			Q_strncpyz( pendingBotAdds[n].team, team ? team : "", sizeof(pendingBotAdds[n].team) );
			pendingBotAdds[n].delay = delay;
			Q_strncpyz( pendingBotAdds[n].altname, altname ? altname : "", sizeof(pendingBotAdds[n].altname) );
			pendingBotAdds[n].queueTime = level.time;
			return;
		}
	}

	G_Printf( S_COLOR_YELLOW "Unable to queue bot add for '%s', AAS not ready and queue is full\n", name );
}
...
===============
*/
static void G_AddBotNow( const char *name, float skill, const char *team, int delay, char *altname) {
	int				clientNum;
	int				teamNum;
	int				botinfoNum;
	char			*botinfo;
...
Gate on AAS readiness before touching a client slot at all -- calling
G_AddBotNow while botlib is still mid-route-build just fails inside
BotAISetupClient and drops the slot again (see trap_AAS_Initialized checks
in ai_main.c). Queue the request and retry from G_CheckPendingBotAdds
instead of wasting the allocate/drop cycle.
===============
*/
static void G_AddBot( const char *name, float skill, const char *team, int delay, char *altname ) {
	if ( !trap_AAS_Initialized() ) {
		G_QueuePendingBotAdd( name, skill, team, delay, altname );
		return;
	}

	G_AddBotNow( name, skill, team, delay, altname );
}


/*
===============
G_CheckPendingBotAdds
===============
*/
void G_CheckPendingBotAdds( void ) {
	int		n;
	...
	for( n = 0; n < PENDING_BOT_ADD_DEPTH; n++ ) {
		...
		if ( trap_AAS_Initialized() ) {
			pendingBotAdds[n].active = qfalse;
			G_AddBotNow( pendingBotAdds[n].name, pendingBotAdds[n].skill,
				pendingBotAdds[n].team, pendingBotAdds[n].delay, pendingBotAdds[n].altname );
			continue;
		}

		if ( level.time - pendingBotAdds[n].queueTime > PENDING_BOT_ADD_TIMEOUT ) {
			G_Printf( S_COLOR_YELLOW "Dropping queued bot add for '%s', AAS never became ready\n",
				pendingBotAdds[n].name );
			pendingBotAdds[n].active = qfalse;
		}
	}
}
...
void G_InitBots( qboolean restart ) {
	...
	// stale entries would compare against the new map's (reset) level.time
	memset( pendingBotAdds, 0, sizeof(pendingBotAdds) );

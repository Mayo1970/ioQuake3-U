...
/* Native GX2 renderer -- there is no GL and no SDL on Wii U. The image
 * loaders in renderercommon include tr_common.h -> qgl.h but never call a GL
 * function; only the GL scalar typedefs are needed for this header's extern
 * declarations (which stay unreferenced/unlinked) and tr_common.h's image_t.
 */
#include <stdint.h>
#include <stddef.h>
...
#endif
/* WIIU_GX2BE (stock renderergl1 frontend + GX2 backend) compiles code that
 * names GL enum constants; supply the standard values. */
#include "qgl_wiiu_consts.h"
